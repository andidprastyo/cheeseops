...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="text-center mb-12" data-aos="fade-down">
        <h1 class="text-4xl font-bold text-primary mb-4">Proses Start-Up Keju</h1>
        <p class="text-lg text-secondary max-w-3xl mx-auto">
            Berikut adalah tahapan proses Start-Up keju berdasarkan data persiapan produksi.
        </p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-lg border border-primary/10 mb-10 overflow-hidden" data-aos="fade-up">
        <div class="bg-green-600 p-6 text-white flex justify-between items-center">
            <h2 class="text-xl font-semibold">Data Proses Start-Up</h2>
            @if(!$startup)
            <a href="{{ route('process.startup.create', $preparation) }}"
                class="text-green-600 bg-white hover:bg-green-50 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 inline-flex items-center transition-colors duration-300">
                <i class="fas fa-plus-circle mr-2"></i> Tambah Data
            </a>
            @else
            <a href="{{ route('process.startup.edit', $preparation) }}"
                class="text-green-600 bg-white hover:bg-green-50 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-4 py-2 inline-flex items-center transition-colors duration-300">
        ...
        <div class="p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-secondary/20 p-4 rounded-lg">
                    <p class="text-sm font-medium text-primary/70 mb-2">Suhu</p>
                    <p class="text-primary font-semibold">{{ number_format($startup->temperature, 1) }}°C</p>
                </div>
                <div class="bg-secondary/20 p-4 rounded-lg">
                    <p class="text-sm font-medium text-primary/70 mb-2">Analisa</p>
                    <p class="text-primary">{{ $startup->analysis }}</p>
                </div>
            </div>
            @else
            <div class="text-center py-6">
                <div class="bg-secondary/20 rounded-full p-4 inline-block mb-4">
                    <i class="fas fa-info-circle text-primary text-2xl"></i>
                </div>
                <p class="text-primary">Belum ada data startup. Silakan tambahkan data.</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <!-- Action Buttons -->
    <div class="text-center mt-8 space-x-4" data-aos="fade-up">
        ...
        <a href="{{ route('process.shutdown.show', $preparation) }}"
            class="text-white bg-green-600 hover:bg-green-700 focus:ring-4 focus:ring-green-300 font-medium rounded-lg px-6 py-3 inline-flex items-center transition-colors duration-300">
            <i class="fas fa-arrow-right mr-2"></i> Lanjut ke Proses Shutdown
        </a>
    </div>
</div>
@endsection
